...
public:
	...
	// 单例 nginx master 加载调用的接口
	static MMSeg* get_instance(const std::string& dict, const std::string& char_freqs) {
		static MMSeg *Pinstance = NULL;
		if(!Pinstance) {
			Pinstance = new MMSeg;
			Pinstance->load(dict, char_freqs);
		}
		return Pinstance;
	}
public:
