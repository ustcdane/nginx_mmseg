...
#LINK =	$(CC)
LINK =  $(CXX) -std=c++11 -g
CXXFLAGS=$(CFLAGS) -std=c++11 -g -O
MMSEG_PATH=/search/daniel/nginx_mmseg/mmseg/src
ALL_INCS = -I src/core \
	-I src/event \
	-I src/event/modules \
	-I src/os/unix \
	-I /search/daniel/pcre-8.37 \
	-I objs \
	-I src/http \
	-I src/http/modules \
	-I src/mail \
    -I /search/daniel/nginx_mmseg \
    -I $(MMSEG_PATH) \
    -I $(MMSEG_PATH)/util
...
CORE_DEPS = src/core/nginx.h \
	...
	/search/daniel/pcre-8.37/pcre.h \
...
# add by Daniel
ADDON_PATH=/search/daniel/nginx_mmseg
ADDON_DEP = $(ADDON_PATH)/ngx_http_handle_interface.h \
            $(ADDON_PATH)/ngx_http_mmseg_module.h

ADDON_DEPS = $(CORE_DEPS)  $(HTTP_DEPS) $(ADDON_DEP)
...
objs/nginx:	objs/src/core/nginx.o \
	...
	objs/addon/nginx_mmseg/ngx_http_mmseg_module.o \
	objs/addon/nginx_mmseg/handle_interface.o \
	objs/ngx_modules.o \
	/search/daniel/pcre-8.37/.libs/libpcre.a
    ...
    objs/addon/nginx_mmseg/handle_interface.o \
	objs/ngx_modules.o \
	 #-lpthread -lcrypt /search/daniel/pcre-8.37/.libs/libpcre.a -lz
	#-L$(MMSEG_PATH)/../build -lmmseg -lpthread -lcrypt /search/daniel/pcre-8.37/.libs/libpcre.a -lz
...
# add by Daniel HTTP model

objs/addon/nginx_mmseg/ngx_http_mmseg_module.o:   $(ADDON_DEPS) \
	/search/daniel/nginx_mmseg/ngx_http_mmseg_module.c
	$(CC) -c $(CFLAGS)  $(ALL_INCS) \
        -o objs/addon/nginx_mmseg/ngx_http_mmseg_module.o \
        /search/daniel/nginx_mmseg/ngx_http_mmseg_module.c

objs/addon/nginx_mmseg/handle_interface.o:   $(ADDON_DEPS) \
    /search/daniel/nginx_mmseg//ngx_http_handle_interface.cpp 
	$(CXX) -c $(CXXFLAGS)  $(ALL_INCS) \
        -o objs/addon/nginx_mmseg/handle_interface.o \
        /search/daniel/nginx_mmseg//ngx_http_handle_interface.cpp


/search/daniel/pcre-8.37/pcre.h:	/search/daniel/pcre-8.37/Makefile

/search/daniel/pcre-8.37/Makefile:	objs/Makefile
	cd /search/daniel/pcre-8.37 \
	&& if [ -f Makefile ]; then $(MAKE) distclean; fi \
	&& CC="$(CC)" CFLAGS="-O2 -fomit-frame-pointer -pipe " \
	./configure --disable-shared 

/search/daniel/pcre-8.37/.libs/libpcre.a:	/search/daniel/pcre-8.37/Makefile
	cd /search/daniel/pcre-8.37 \
	&& $(MAKE) libpcre.la
...
objs/nginx.8:	man/nginx.8 objs/ngx_auto_config.h
	sed -e "s|%%PREFIX%%|/search/daniel/nginx-1.8.0/bin|" \
		-e "s|%%PID_PATH%%|/search/daniel/nginx-1.8.0/bin/logs/nginx.pid|" \
		-e "s|%%CONF_PATH%%|/search/daniel/nginx-1.8.0/bin/conf/nginx.conf|" \
		-e "s|%%ERROR_LOG_PATH%%|/search/daniel/nginx-1.8.0/bin/logs/error.log|" \
		< man/nginx.8 > $@

install:	objs/nginx 		
	test -d '$(DESTDIR)/search/daniel/nginx-1.8.0/bin' || mkdir -p '$(DESTDIR)/search/daniel/nginx-1.8.0/bin'

	test -d '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/sbin' 		|| mkdir -p '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/sbin'
	test ! -f '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/sbin/nginx' 		|| mv '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/sbin/nginx' 			'$(DESTDIR)/search/daniel/nginx-1.8.0/bin/sbin/nginx.old'
	cp objs/nginx '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/sbin/nginx'

	test -d '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf' 		|| mkdir -p '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf'

	cp conf/koi-win '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf'
	cp conf/koi-utf '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf'
	cp conf/win-utf '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf'

	test -f '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/mime.types' 		|| cp conf/mime.types '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf'
	cp conf/mime.types '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/mime.types.default'

	test -f '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/fastcgi_params' 		|| cp conf/fastcgi_params '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf'
	cp conf/fastcgi_params 		'$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/fastcgi_params.default'

	test -f '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/fastcgi.conf' 		|| cp conf/fastcgi.conf '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf'
	cp conf/fastcgi.conf '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/fastcgi.conf.default'

	test -f '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/uwsgi_params' 		|| cp conf/uwsgi_params '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf'
	cp conf/uwsgi_params 		'$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/uwsgi_params.default'

	test -f '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/scgi_params' 		|| cp conf/scgi_params '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf'
	cp conf/scgi_params 		'$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/scgi_params.default'

	test -f '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/nginx.conf' 		|| cp conf/nginx.conf '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/nginx.conf'
	cp conf/nginx.conf '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/conf/nginx.conf.default'

	test -d '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/logs' 		|| mkdir -p '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/logs'

	test -d '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/logs' || 		mkdir -p '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/logs'

	test -d '$(DESTDIR)/search/daniel/nginx-1.8.0/bin/html' 		|| cp -R html '$(DESTDIR)/search/daniel/nginx-1.8.0/bin'
